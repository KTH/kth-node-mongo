  ...
  return new Promise(resolve => {
    mongoose.connection.on('error', error => {
      log.fatal(
        ...
        { error: error.message, dbOptions, MANUAL_RECONNECT_TIMEOUT }
      )
    ...
    mongoose.connection.on('disconnected', () => {
      ...
      _manualReconnectAfterSomeDelay({
        ...
        log,
        logWithFatal: true,
        logText: 'Missing automatic reconnect',
      })
    })
...
function _manualReconnectAfterSomeDelay({ dbUri, dbOptions, log, logWithFatal, logText }) {
  ...
  const _reconnect = () => {
    const _logText =
      'kth-node-mongo: Looks like automatic reconnect is not working' +
      ' - attempting to reconnect manually' +
      (logText ? ` (${logText})` : '')
