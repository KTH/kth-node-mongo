  ...
  return new Promise(resolve => {
    mongoose.connection.on('error', dbErr => {
      ...
      log.fatal(
        ...
        { alias: 'DB connection error', err: dbErr.message, delayMilliseconds, keepAlive }
      )
    ...
    mongoose.connection.on('disconnected', () => {
      log.fatal('kth-node-mongo: Default connection lost', { alias: 'DB connection disconnected' })
      Global.isConnected = false
    })

    mongoose.connection.on('reconnected', () => {
      log.info('kth-node-mongo: Default connection re-established', {
        alias: 'DB connection reconnected',
      })
      Global.isConnected = true
    })

    mongoose.connection.on('reconnectFailed', () => {
      log.fatal('kth-node-mongo: Could not reconnect to DB', {
        alias: 'DB connection reconnectFailed',
      })
      Global.isConnected = false
    })
